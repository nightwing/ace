...
var DiffComputer = require("./vscode-diff/dist/diffComputer").DiffComputer;
...
class DiffView {
    ...
    onConnectorScroll(ev) {
        ...
        if (dir < 0) {
            ...
            else if (mid < (isOrig ? ch.origEnd : ch.editEnd)) ch = chunks[i - 1] || ch;
            else ch = chunks[i];
        } else {
            ch = chunks[i + 1] || {
                editStart: r2.session.getLength(),
        ...
        this.$syncScroll = true;
        if (
            Math.abs(scrollTop1 - r1.session.getScrollTop()) <= 1 &&
    ...
    $initArrow() {
        ...
        this.gutterEl.addEventListener(
            "mousemove",
            function(e) {
