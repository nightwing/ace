...
var computeDiff = require("./diff-lib").computeDiff;
    ...
    $diffLines(val1, val2) {
        ...
        if (chunks.changes) {
            return chunks.changes.map((changes) => {
                let originalStartLineNumber = changes.original.startLineNumber - 1;
                let originalEndLineNumber = changes.original.endLineNumberExclusive - 1;
                let modifiedStartLineNumber = changes.modified.startLineNumber - 1;
                let modifiedEndLineNumber = changes.modified.endLineNumberExclusive - 1;
...
class DiffHighlight {
    ...
    update(html, markerLayer, session, config) {
        ...
        //TODO: hack for decorators to be forcefully updated until we got new change type in VirtualRenderer
