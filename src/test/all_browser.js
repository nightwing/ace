...
function testLink(file, testname) {
    var href = '?' + file + (useMockdom ? "&mock=1" : "")
        + (test ? "")
    return ["a", {href:}, file.replace(/^ace\//, "") + ".js"];
}
function normalizeHref(str) {
    return str.replace(/([?&])&+/g, "$1");
}

var nav = buildDom(["div", {style: "position:absolute;right:0;top:0;;height:100vh;overflow:auto"}, html], document.documentElement);
var log = buildDom(["div", {id: "log", style: "position:absolute;left:0;top:0;height:100vh;overflow:auto"}], document.documentElement);
...
require(selectedTests, async function() {
    ...
    var watcherId;
    function watch() {
        if (!watcherId) setInterval(watch, 50);
    ...
    async function runTimed(step, callback) {
        ...
        var callFailed = true;
        try {
            (fn.length ? fn.call(step.testSuite, done) : fn.call(step.testSuite));
            callFailed = false;
        } finally {
            if (!fn.length)
                done(callFailed)
